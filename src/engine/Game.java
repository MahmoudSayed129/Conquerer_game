...
public class Game {
	...
	public void targetCity(Army army, String targetName) {
       // System.out.println("target");
		...
		for (Distance d : distances) {
			...
			if ((d.getFrom().equalsIgnoreCase(from) || d.getFrom().equalsIgnoreCase(targetName))
					&& (d.getTo().equalsIgnoreCase(from) || d.getTo().equalsIgnoreCase(targetName))) {
				...
				//System.out.println("target "+army.getCurrentLocation()+"     "+army.getCurrentStatus());
			}
		}
	...
	public void endTurn() {
		...
		for (Army a : player.getControlledArmies()) {
			...
			if (a.getDistancetoTarget() == 0 && a.getCurrentStatus() == Status.MARCHING) {
